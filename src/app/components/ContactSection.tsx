      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        ...
        <div className="max-w-xl mb-12">
          ...
          <h2 className="reveal delay-100 text-section-xl font-serif text-primary">
            Find{' '}
            <span className="teal-gradient-text italic">Us</span>
          </h2>
          <p className="reveal delay-200 text-muted-foreground text-base leading-relaxed mt-4">
            Conveniently located in the heart of Begusarai, Bihar — easily accessible from Patna, Darbhanga, and Muzaffarpur.
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          {/* Left: Contact cards */}
          <div className="lg:col-span-4 flex flex-col gap-5 reveal-left">
            {[
              {
                icon: 'MapPinIcon',
              ...
              <div key={item.title} className="card-hover flex items-start gap-4 p-5 bg-card rounded-2xl border border-border shadow-card">
                ...
                <div>
                  ...
                  {item.lines.map((line, i) => (
                    <div key={i} className="text-muted-foreground text-sm">{line}</div>
                  ))}
                </div>
              </div>
            ))}
            ...
            <div className="flex gap-3 mt-2">
              {[
                { icon: 'GlobeAltIcon', label: 'YouTube', href: 'https://www.youtube.com/@DrPrashantKashyap-Cardiologist' },
                ...
                <a
                  ...
                  aria-label={s.label}
                  className="w-10 h-10 rounded-xl bg-card border border-border flex items-center justify-center text-muted-foreground hover:text-accent hover:border-accent/40 transition-all"
                >
          ...
          <div className="lg:col-span-8 reveal-right">
            <div className="rounded-3xl overflow-hidden border border-border shadow-card h-96 lg:h-full min-h-96 relative">
              <iframe
              ...
              />
              {/* Map overlay label */}
              <div className="absolute top-4 left-4 glass-card-light rounded-xl px-4 py-2.5 shadow-card">
                <div className="text-primary font-semibold text-sm">Prashant Heart Hospital</div>
                <div className="text-muted-foreground text-xs mt-0.5">IMA Path, Begusarai, Bihar</div>
              </div>
            </div>
          </div>
        </div>
